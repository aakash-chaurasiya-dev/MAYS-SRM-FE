...
export default function CreateInvoicePage() {
  ...
  /*
   * false = default view
   *         only Receivable rows
   *
   * true = Full Details
   *        Receivable + Payable rows
   */
  ...
  const { data: products = [] } = useQuery({
    ...
    queryFn: async () => {
      ...
      return data.map((p, idx) => ({
        ...
        productId:
          p.partCatId || p.productId,

        productName:
          p.partName || p.productName,
    ...
    if (ticketData) {
      setForm((f) => ({
        ...
        customerName:
          ...
          ' ' +
          (ticketData.userLastName || ''),

        contactNumber:
          ticketData.userMobileNo || '',

        deviceName:
          ticketData.deviceModelName || '',
      }));
    }
  }, [ticketData]);
  ...
  const canUserSeeChargeType = (
    chargeType
  ) => {
    if (!chargeType) return false;

    /*
     * null / empty allowedRoles
     * means everybody can see it.
     */
    if (
      !chargeType.allowedRoles?.trim()
    ) {
      return true;
    }

    const allowedRoles =
      chargeType.allowedRoles
        .split(',')
        .map((role) =>
          role.trim()
        )
    ...
    try {
      const payload = items.map(
        (i) => ({
          ...
          /*
           * IMPORTANT:
           * Send original positive amount.
           * Do NOT send payable as negative.
           */
          amount:
            Math.abs(
              Number(i.amount || 0)
      ...
      await api.put(
        `/billing/ticket/${ticketId}/charges`,
        payload
      );
  ...
  return (
    <Box>
      ...
      <InvoicePreview
         ...
         * IMPORTANT:
         *
         * Use visibleItems here rather than
         * all items, so preview follows the
         * same R/P + role visibility rules.
         */
