...
export default function DepartmentManagementPage() {
  ...
  const fetchDepartments = useCallback(async () => {
    try {
      ...
      setDepartments(data.map((dept, index) => ({
        ...dept,
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (modalMode === 'create') {
        ...
        await api.put(`/departments/${formData.departmentId}`, payload);
        setSelectedIds([]); 
    ...
    <Box sx={{ p: 2 }}>
      ...
      <Box sx={{ mb: 3 }}>
        ...
        <Typography sx={{ fontSize: '14px', color: theme.palette.text.secondary }}>
          Manage organizational units such as Hardware, Software, and Logistics
        </Typography>
      </Box>
      ...
      >
        ...
        <DialogActions sx={{ px: 3, py: 2 }}>
          ...
          <Button type="submit" form="department-form" variant="contained" disabled={submitLoading} sx={{ px: 3, minWidth: 100 }}>
            {submitLoading ? <CircularProgress size={24} color="inherit" /> : (modalMode === 'create' ? 'Save Department' : 'Update Department')}
          </Button>
        </DialogActions>
      </Dialog>
      ...
      <Dialog open={openDeleteConfirm} onClose={() => setOpenDeleteConfirm(false)} maxWidth="sm" fullWidth>
        ...
        <DialogContent>
          <DialogContentText>
            Are you sure you want to delete {selectedIds.length === 1 ? 'this department' : `these ${selectedIds.length} departments`}? This action cannot be undone.
          </DialogContentText>
        </DialogContent>
        <Divider />
