...
import ForceChangePasswordDialog from './ForcePasswordChangeDialog'; // ← NEW
...
export default function LoginPage() {
  ...
  const [showForceChange, setShowForceChange] = useState(false); // ← NEW
  ...
  const handleMobileChange = (e) => {
    ...
    else if (digits.length < 10) setMobileError('Mobile number must be exactly 10 digits');
    else if (!validateMobile(digits)) setMobileError('Mobile number must start with 6, 7, 8 or 9');
    else setMobileError('');
  };
  ...
  const handleLogin = async (e) => {
    ...
    // ← NEW: backend sends FORCE_PASSWORD_CHANGE for first-time-login users
    if (result.error === 'FORCE_PASSWORD_CHANGE') {
      setIsLoading(false);
  ...
  const handleForceChangeSuccess = () => {
    ...
    setPassword('');
    setError('');
    // Show a friendly nudge to re-login with the new password
    navigate('/login', {
      replace: true,
      state: { infoMessage: 'Password changed successfully. Please sign in with your new password.' },
    });
  };
